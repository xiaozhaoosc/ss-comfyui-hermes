#!/usr/bin/env python3
"""字幕叠加 + 拼接 (相对路径避免中文/冒号问题)"""
import os, subprocess, shutil, tempfile

COMFYUI_DIR = r"D:\ai_projects\ComfyUI"
FINAL_DIR = os.path.join(COMFYUI_DIR, "output", "zombie_girlfriend", "final")
FONT = "output/zombie_girlfriend/msyh.ttc"  # 相对路径,无冒号
...
for scene_num, text in SCENES.items():
    ...
    # 用 ASCII 临时路径做输出
    tmp_out = os.path.join(tmpdir, f"sub_{scene_num:02d}.mp4")
    ...
    result = subprocess.run(cmd, capture_output=True, timeout=60, cwd=COMFYUI_DIR)

    if result.returncode == 0 and os.path.exists(tmp_out) and os.path.getsize(tmp_out) > 0:
        final_sub = os.path.join(FINAL_DIR, f"scene_{scene_num:02d}_sub.mp4")
        shutil.copy2(tmp_out, final_sub)
        ok += 1
        print(f"  OK scene_{scene_num:02d}", flush=True)
    else:
        err = result.stderr.decode("utf-8", "ignore")[-200:] if result.stderr else ""
        print(f"  FAIL scene_{scene_num:02d}: {err}", flush=True)

shutil.rmtree(tmpdir, ignore_errors=True)
print(f"  字幕: {ok}/20", flush=True)
...
for ep_num, (ep_title, sr) in episodes.items():
    ...
    with open(concat_file, "w") as f:
        for sf in files:
            f.write(f"file '{sf}'\n")
    out = os.path.join(FINAL_DIR, f"{ep_title}.mp4")
    cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_file,
           "-c:v", "libx264", "-c:a", "aac", "-pix_fmt", "yuv420p", "-movflags", "+faststart", out]

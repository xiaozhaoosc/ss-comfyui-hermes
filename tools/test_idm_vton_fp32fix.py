...
print('[FIX1] GroupNorm float32')

# === FIX 2: Attention scale fix — use float32 for softmax ===
# Patch the diffusers Attention.get_attention_scores to use float32 softmax
from diffusers.models.attention_processor import Attention
_orig_get_attn = Attention.get_attention_scores
def _safe_get_attn(self, query, key, attention_mask=None):
    dtype = query.dtype
    ...
    attention_scores = torch.baddbmm(
        ...
        query,
        key.transpose(-1, -2),
        beta=0, alpha=self.scale if hasattr(self, 'scale') else (query.shape[-1] ** -0.5),
    )
    if attention_mask is not None:
        attention_scores = attention_scores + attention_mask
    attention_probs = torch.nn.functional.softmax(attention_scores, dim=-1).to(dtype)
    return attention_probs
Attention.get_attention_scores = _safe_get_attn
print('[FIX2] Attention softmax in float32')

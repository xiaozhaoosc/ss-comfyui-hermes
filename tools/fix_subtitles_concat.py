...
FINAL_DIR = r"D:\ai_projects\ComfyUI\output\zombie_girlfriend\final"
...
for scene_num, text in SCENES.items():
    ...
    if not os.path.exists(input_file):
        print(f"  skip scene_{scene_num:02d}", flush=True)
        continue
    ...
    if result.returncode == 0:
        ...
        print(f"  FAIL scene_{scene_num:02d}: {result.stderr[-200:]}", flush=True)
...
for ep_num, (ep_title, scene_range) in episodes.items():
    ...
    for sn in scene_range:
        sub_file = os.path.join(FINAL_DIR, f"scene_{sn:02d}_sub.mp4")
        normal_file = os.path.join(FINAL_DIR, f"scene_{sn:02d}.mp4")
    ...
    if not scene_files:
        print(f"  skip {ep_title}", flush=True)
        continue

...
def create_api_workflow(prompt, seed=None):
    ...
    return {
        ...
        "6": {"class_type": "Wan22ImageToVideoLatent", "inputs": {"vae": ["3", 0], "width": 640, "height": 384, "length": 41, "batch_size": 1}},
        "7": {"class_type": "KSampler", "inputs": {"model": ["1", 0], "positive": ["4", 0], "negative": ["5", 0], "latent_image": ["6", 0], "seed": seed, "control_after_generate": "fixed", "steps": 20, "cfg": 6.5, "sampler_name": "euler", "scheduler": "normal", "denoise": 1}},
        "8": {"class_type": "VAEDecode", "inputs": {"samples": ["7", 0], "vae": ["3", 0]}},
        "9": {"class_type": "SaveImage", "inputs": {"images": ["8", 0], "filename_prefix": f"zombie_{seed}"}},
        "10": {"class_type": "SaveAnimatedWEBP", "inputs": {"images": ["8", 0], "filename_prefix": f"zombie_{seed}", "fps": 8, "lossless": False, "quality": 80, "method": "default"}}
    }

def queue_prompt(workflow):
    p = {"prompt": workflow}
    data = json.dumps(p).encode('utf-8')
    req = requests.post(f"{COMFYUI_URL}/prompt", data=data, timeout=30)
    return json.loads(req.text)

def get_history(prompt_id):
    req = requests.get(f"{COMFYUI_URL}/history/{prompt_id}", timeout=10)
    return json.loads(req.text)
        ...
        if 'images' in node_output:
            for img_info in node_output['images']:
                ...
                if filename.endswith('.webp'):
                    url = f"{COMFYUI_URL}/view?filename={filename}&subfolder={subfolder}&type=output" if subfolder else f"{COMFYUI_URL}/view?filename={filename}&type=output"
                    req = requests.get(url, timeout=30)
                    if req.status_code == 200:
                        output_filename = f"scene_{scene_num:02d}_{title}.webp"
                        output_path = os.path.join(OUTPUT_DIR, output_filename)
                        with open(output_path, 'wb') as f:
                            f.write(req.content)
                        print(f"  ✅ scene_{scene_num:02d} ({len(req.content)//1024}KB)", flush=True)
                        return output_path
    print(f"  ❌ 未找到视频", flush=True)
    return None

def webp_to_mp4(webp_path):
    mp4_path = webp_path.replace('.webp', '.mp4')
    cmd = ['ffmpeg', '-y', '-i', webp_path, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', mp4_path]
...
def main():
    print("🧟 《我的女友是丧尸》批量视频生成", flush=True)
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    # 检查已完成的场景
    existing = set()
    for f in os.listdir(OUTPUT_DIR):
        if f.startswith('scene_') and (f.endswith('.webp') or f.endswith('.mp4')):
            num = int(f.split('_')[1])
            existing.add(num)
    print(f"已完成: {sorted(existing)}", flush=True)
    
    results = []
    for scene_num, scene_info in SCENE_PROMPTS.items():
        if scene_num in existing:
            print(f"⏭ 跳过场景 {scene_num}: {scene_info['title']} (已存在)", flush=True)
            results.append({'scene': scene_num, 'title': scene_info['title'], 'status': 'skipped'})
            continue
        
        print(f"🎬 场景 {scene_num:02d}/20: {scene_info['title']}", flush=True)
        try:
            ...
            if outputs:
                ...
                if video_path:
                    mp4_path = webp_to_mp4(video_path)
                    results.append({'scene': scene_num, 'title': scene_info['title'], 'path': mp4_path, 'status': 'success'})
                else:
                    results.append({'scene': scene_num, 'title': scene_info['title'], 'status': 'download_failed'})
            else:
                print(f"  ⏰ 超时", flush=True)
                results.append({'scene': scene_num, 'title': scene_info['title'], 'status': 'timeout'})
        except Exception as e:
            print(f"  ❌ 失败: {e}", flush=True)
            results.append({'scene': scene_num, 'title': scene_info['title'], 'status': 'error', 'error': str(e)})
        
        time.sleep(1)
    
    success_count = sum(1 for r in results if r['status'] in ('success', 'skipped'))
    print(f"\n📊 成功: {success_count}/20", flush=True)

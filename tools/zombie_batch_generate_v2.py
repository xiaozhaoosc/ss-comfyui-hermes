...
def convert_workflow_to_api_format(workflow):
    ...
    for node in workflow['nodes']:
        ...
        if 'widgets_values' in node:
            # 根据节点类型映射 widgets 到 inputs
            if node['type'] == 'UNETLoader':
                api_format[node_id]['inputs']['unet_name'] = node['widgets_values'][0]
                api_format[node_id]['inputs']['weight_dtype'] = node['widgets_values'][1]
            elif node['type'] == 'DualCLIPLoader':
                api_format[node_id]['inputs']['clip_name1'] = node['widgets_values'][0]
                api_format[node_id]['inputs']['clip_name2'] = node['widgets_values'][1]
                api_format[node_id]['inputs']['type'] = node['widgets_values'][2]
            elif node['type'] == 'VAELoader':
                api_format[node_id]['inputs']['vae_name'] = node['widgets_values'][0]
            elif node['type'] == 'CLIPTextEncode':
                if 'Positive' in node.get('title', ''):
                    api_format[node_id]['inputs']['text'] = node['widgets_values'][0]
                elif 'Negative' in node.get('title', ''):
                    api_format[node_id]['inputs']['text'] = node['widgets_values'][0]
            elif node['type'] == 'EmptyLatentImage':
                api_format[node_id]['inputs']['width'] = node['widgets_values'][0]
                api_format[node_id]['inputs']['height'] = node['widgets_values'][1]
                api_format[node_id]['inputs']['batch_size'] = node['widgets_values'][2]
            elif node['type'] == 'WanImageToVideo':
                api_format[node_id]['inputs']['height'] = node['widgets_values'][0]
                api_format[node_id]['inputs']['width'] = node['widgets_values'][1]
                api_format[node_id]['inputs']['length'] = node['widgets_values'][2]
                api_format[node_id]['inputs']['batch_size'] = node['widgets_values'][3]
            elif node['type'] == 'KSampler':
                api_format[node_id]['inputs']['seed'] = node['widgets_values'][0]
                api_format[node_id]['inputs']['steps'] = node['widgets_values'][1]
                api_format[node_id]['inputs']['cfg'] = node['widgets_values'][2]
                api_format[node_id]['inputs']['sampler_name'] = node['widgets_values'][3]
                api_format[node_id]['inputs']['scheduler'] = node['widgets_values'][4]
                api_format[node_id]['inputs']['denoise'] = node['widgets_values'][5]
            elif node['type'] == 'VHS_VideoCombine':
                api_format[node_id]['inputs']['video_format'] = node['widgets_values'][0]
                api_format[node_id]['inputs']['frame_rate'] = node['widgets_values'][1]
                api_format[node_id]['inputs']['loop_count'] = node['widgets_values'][2]
                api_format[node_id]['inputs']['filename_prefix'] = node['widgets_values'][3]
                api_format[node_id]['inputs']['type'] = node['widgets_values'][4]
                ...
                if 'link' in inp and inp['link'] is not None:
                    ...
                    for link in workflow['links']:
                        if link[0] == inp['link']:
                            ...
                            from_slot = link[2]
                            api_format[node_id]['inputs'][inp['name']] = [from_node, from_slot]
                            break
    
    return api_format

def update_prompt_in_api_format(api_format, prompt):
    """更新 API 格式中的 prompt"""
    # 找到正向 prompt 节点
    for node_id, node_data in api_format.items():
        if node_data['class_type'] == 'CLIPTextEncode':
            # 通过检查是否是第一个 CLIPTextEncode 节点来判断
            if 'text' in node_data['inputs']:
                # 更新第一个找到的 CLIPTextEncode 节点
                node_data['inputs']['text'] = prompt
                break
    return api_format

def queue_prompt(api_format):
    """提交 prompt 到 ComfyUI"""
    p = {"prompt": api_format}
...
def main():
    ...
    api_format_template = convert_workflow_to_api_format(workflow_template)
    ...
    for scene_num, scene_info in SCENE_PROMPTS.items():
        ...
        # 更新 prompt
        api_format = update_prompt_in_api_format(api_format_template.copy(), scene_info['prompt'])
        
        # 提交生成任务
        try:
            result = queue_prompt(api_format)

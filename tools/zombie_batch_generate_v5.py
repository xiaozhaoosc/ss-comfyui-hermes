            ...
            for img_info in node_output['images']:
                ...
                if filename.endswith('.webp'):
                    ...
                    if req.status_code == 200:
                        ...
                        print(f"✅ 下载完成: {output_filename}", flush=True)
                        return output_path
    
    print(f"❌ 未找到场景 {scene_num} 的视频文件", flush=True)
    return None
...
def main():
    ...
    for scene_num, scene_info in SCENE_PROMPTS.items():
        print(f"\n🎬 生成场景 {scene_num}: {scene_info['title']}", flush=True)
        
        try:
            workflow = create_api_workflow(scene_info['prompt'])
            result = queue_prompt(workflow)
            prompt_id = result['prompt_id']
            print(f"   提交成功，ID: {prompt_id}", flush=True)
